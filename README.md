...
2. Merge `templates/AGENTS.md` into target repo root `AGENTS.md` non-destructively.
3. Merge `templates/docs/agent/**` into target repo as `docs/agent/**` non-destructively.
4. Merge `templates/docs/human/**` into target repo as `docs/human/**` non-destructively.
5. Merge `templates/docs/README.md` into target repo as `docs/README.md` non-destructively.
6. Merge `templates/.agents/skills/**` into target repo as `.agents/skills/**` non-destructively.
7. Never replace existing instruction files wholesale when they already exist; append or edit-in-place while preserving existing repo rules.
8. Run skill `agent-kit-repo-adjuster` in the target repo.
9. Confirm no placeholder values remain (for example `<set-client-path>`).
10. Explain to the human operator how to run this pattern in day-to-day work (plan, execute, review, verify, close), using `docs/human/*`.
